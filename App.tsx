...
import { Plane, Heart, Clock, Calendar, CheckCircle, RefreshCw, LogOut, MapPin, Utensils, Moon, Car, Settings, BarChart, Trash2, Plus, Edit2, X, Save, Share2, Info } from 'lucide-react';
import { FlightShift, CareTask, FamilyMember } from './types';
...
const App: React.FC = () => {
  ...
  const handleSync = async () => {
    setIsSyncing(true);
    // סימולציה של שליפת יומן - במציאות כאן יבוא ה-API של Google Calendar
    const mockEvents = [
          ...
          <div className="space-y-6 animate-in fade-in duration-300 pb-10">
              ...
              {shifts.map(shift => (
                <div key={shift.id} className="bg-white p-5 rounded-[32px] border border-slate-200 shadow-sm space-y-4">
                  ...
                  <div className="pt-2 border-t border-slate-50 space-y-2">
                    {tasks.filter(t => t.shiftId === shift.id).map(task => (
                      <div key={task.id} className="flex items-center justify-between bg-slate-50 p-3 rounded-2xl border border-slate-100">
                        {editingTaskId === task.id ? (
                           <div className="flex-1 flex gap-2">
                             <input value={editTaskForm.description} onChange={e => setEditTaskForm({...editTaskForm, description: e.target.value})} className="flex-1 bg-white p-2 rounded-lg text-[10px] font-bold border" />
                             <input value={editTaskForm.time} onChange={e => setEditTaskForm({...editTaskForm, time: e.target.value})} className="w-16 bg-white p-2 rounded-lg text-[10px] font-bold border text-center" />
                             <button onClick={saveTaskEdit} className="bg-indigo-500 text-white p-2 rounded-lg"><Save size={12}/></button>
                           </div>
                        ) : (
                          <>
                            <div className="flex items-center gap-2">
                              {getTaskIcon(task.taskType, 16)}
                              <span className="text-xs font-bold text-slate-700">{task.description} ({task.time})</span>
                            </div>
                            <button onClick={() => startEditingTask(task)} className="text-indigo-400 p-1"><Edit2 size={14}/></button>
                          </>
                        )}
                      </div>
                    ))}
            ...
            <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm space-y-4">
               <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-black text-slate-800 uppercase text-sm">סטטיסטיקת עזרה לעלמא</h3>
               </div>
               <div className="space-y-5">
                 {FAMILY_MEMBERS.map(member => {
                   const count = tasks.filter(t => t.assignedTo === member.id).length;
                   ...
                   return (
                     <div key={member.id} className="space-y-2">
                        <div className="flex justify-between items-center text-xs font-bold text-slate-600">
                          ...
                          <span className="bg-slate-50 px-2 py-0.5 rounded-lg border">{count} משימות</span>
                        </div>
                        <div className="h-3 bg-slate-50 rounded-full overflow-hidden border border-slate-100 relative shadow-inner">
                           <div className="h-full bg-rose-500 transition-all duration-1000 ease-out" style={{ width: `${percentage}%` }}></div>
                        </div>
                     </div>
                   );
                 })}
            ...
            <div className="bg-slate-800 text-white p-6 rounded-[32px] shadow-xl flex items-center justify-between">
               <div>
                  <p className="font-black text-lg leading-tight italic">אירוח חינם (PWA)</p>
                  <p className="text-[10px] font-medium opacity-60">הוסף למסך הבית כדי לקבל חווית אפליקציה מלאה</p>
               </div>
               <Info size={32} className="text-rose-400 opacity-40" />
            </div>
          </div>
        )}
      </main>

      {/* תפריט ניווט תחתון מותאם מובייל */}
